...
const LibraryPage: React.FC = () => {
	...
	const filteredBooks = books.filter((book) => {
		...
		const matchesSearch =
			...
			(book.categories &&
				book.categories.toLowerCase().includes(searchLower)) ||
			(book.publisher && book.publisher.toLowerCase().includes(searchLower))

		const matchesCategory =
			!filters.category ||
			(book.categories &&
				book.categories.toLowerCase().includes(filters.category.toLowerCase()))
		...
		const matchesOwned = !filters.owned || book.owned === true

		return (
			matchesSearch && matchesCategory && matchesPublisher && matchesLanguage && matchesOwned
		)
	})
	...
	const sortedBooks = [...filteredBooks].sort((a, b) => {
		const getPriority = (book: any) => {
			if (!book.finish_date && !book.want_to_read && !book.library_only)
				return 1 // Currently Reading
	...
	const getStatusBadge = (book: any) => {
		...
		if (!book.finish_date && !book.want_to_read && !book.library_only)
			return { text: 'Currently Reading', color: 'badge-warning' }
	...
	return (
		<div className='space-y-8'>
			...
			<div className='text-center mb-8'>
				<Link
					to='/add-book'
					className='btn btn-primary btn-lg btn-wide text-lg'
				>
					➕ Add New Book
				</Link>
				<p className='text-base-content/70 mt-2 text-sm'>
					Scan a barcode or search for books to add to your library
				</p>
			</div>
			...
			<div className='bg-base-100 border-2 border-secondary rounded-2xl p-4 mb-6 shadow-lg'>
				<div className='flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4'>
					...
					<div className='lg:hidden'>
						...
						>
							<Icon
								hero={<FunnelIcon className='w-4 h-4 mr-2' />}
								emoji='⚙️'
							/>
							Filters
						</button>
						...
						<div className='form-control w-full'>
							<label className='label cursor-pointer justify-start gap-3'>
								<input
									...
									onChange={(e) => setFilters((prev) => ({ ...prev, owned: e.target.checked }))}
								/>
								<span className='label-text font-semibold'>Owned only</span>
							</label>
						</div>
					</div>
					...
					<div className='modal-box w-11/12 max-w-2xl'>
						...
						<div className='space-y-4'>
							...
							<SearchableDropdown
								options={[
									'',
									...publishers.filter((p): p is string => Boolean(p)),
								]}
								value={filters.publisher ?? ''}
								onChange={(value) =>
									setFilters((prev) => ({ ...prev, publisher: value }))
								}
							...
							<SearchableDropdown
								options={[
									'',
									...languages.filter((l): l is string => Boolean(l)),
								]}
								value={filters.language ?? ''}
								onChange={(value) =>
			...
			<div className='flex justify-between items-center mb-4'>
				<div className='flex items-center gap-2'>
					...
					>
						<Icon
							hero={<Squares2X2Icon className='w-4 h-4' />}
							emoji='🔳'
						/>
						Grid
					</button>
					...
					>
						<Icon
							hero={<ListBulletIcon className='w-4 h-4' />}
							emoji='📋'
						/>
						List
					</button>
						...
						<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4'>
							{sortedBooks.map((book) => {
								...
								return (
									<div
										key={book.uid}
										className='book-card bg-base-100 border-2 border-secondary rounded-lg p-3 transition-all duration-300 hover:scale-105 hover:-translate-y-1 hover:shadow-xl cursor-pointer flex flex-col mx-auto h-80 w-full max-w-[200px] relative pb-6'
									>
													...
													onError={(e) => {
														...
														if (
															...
															placeholder.className =
																'w-full h-full bg-base-200 rounded flex items-center justify-center text-2xl'
													...
													<div className='category-badges mb-2 flex flex-wrap gap-1'>
														{book.categories
															...
															.map((category, index) => (
																<span
																	key={index}
																	className='badge badge-sm bg-secondary text-secondary-content border border-primary text-xs px-2 py-1 truncate max-w-full'
																>
																	{category.trim()}
																</span>
															))}
										...
										<div className='absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1/2 z-10 text-center'>
											<span
												className={`badge badge-sm ${statusBadge.color} whitespace-nowrap h-fit`}
											>
												{statusBadge.text}
											</span>
										</div>
						...
						<div className='space-y-4'>
							{sortedBooks.map((book) => {
								...
								return (
									<div
										key={book.uid}
										className='bg-base-100 border-2 border-secondary rounded-lg p-4 hover:shadow-lg transition-all duration-200'
									>
													...
													<div className='flex flex-wrap gap-1 mt-2'>
														{book.categories
															...
															.map((category, index) => (
																<span
																	key={index}
																	className='badge badge-sm bg-secondary text-secondary-content'
																>
																	{category.trim()}
																</span>
															))}
